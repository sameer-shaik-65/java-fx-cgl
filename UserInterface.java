...
import javafx.event.ActionEvent; 
import conwayJavaFX.ConwayMain;
...
public class UserInterface  {
	...
	**********************************************************************************************/
	...
	public UserInterface(Pane theRoot) {
		...
		//button_Load.setOnAction((event) -> { System.out.println(ba.printBoard(ba.createBoard(5,l))); });
		button_Load.setOnAction((event) -> { loadImageData(); });
		//button_Load.setOnAction((event) -> { System.out.println("HEllo");});
	...
	**********************************************************************************************/
	...
	public void loadImageData() {
		//(Scanner s = new Scanner(new File(str_FileName))) 
		try{
			// Your code goes here......
			Scanner s=new Scanner(new File(str_FileName));
			...
			int len=lis.size()/2;
			this.l=new int[len][2];
			int c=0;
			for(int i=0;i<len;i++) {
			  this.l[i][0] = lis.get(c++);
	          this.l[i][1] = lis.get(c++);
			
			}
			
			System.out.println(Arrays.deepToString(this.l));
			oddGameBoard.createBoard(100,this.l);
			pC(oddCanvas);
		/*	
			...
			Group g=new Group(sa);*/
		//	Group ga=new Group(rect_middle);
			//window.getChildren().add(g);
			//window.getChildren().add(ga);
		...
		//System.out.println(ba.printBoard(ba.createBoard(5,l)));
		}
												/*	int[][] arr = new int[5][2];
	...
	private void startConway() {
		...
		//System.out.println(ba.printBoard(ba.createBoard(10, l)));
	...
	public void pC(Pane p)  {
		//Board b=new Board();
		//int[][] l= {{1,3},{1,4},{2,4},{5,3},{5,4},{6,2},{6,3},{7,5},{8,4}};
		//System.out.println(Arrays.deepToString(this.l));
		String dc=oddGameBoard.printBoard(oddGameBoard.createBoard(100,this.l));
		String[] aa=dc.split("\n");
		for(int x=0;x<aa.length-1;x++) {
			for(int y=0;y<aa.length-1;y++) {
				if(aa[x].charAt(y)=='*') {
					Rectangle rec= new Rectangle(5,5,Color.BLACK);
					rec.relocate(6*x,6*y);
	...
	/*	int[][] l= {{1,3},{1,4},{2,4},{5,3},{5,4},{6,2},{6,3},{7,5},{8,4}};
		String sa=oddGameBoard.printBoard(oddGameBoard.createBoard(10,l));
		//System.out.println(sa);
		int aaa=0;
		String[] lines=sa.split("\n");
	...
	public void runSimulation(){
		...
		//int[][] l= {{1,3},{1,4},{2,4},{5,3},{5,4},{6,2},{6,3},{7,5},{8,4}};
		boolean[][] aa=oddGameBoard.createBoard(100,this.l);
		...
		if(toggle) {
			...
			oddGameBoard.generateNextGeneration(aa,5);
